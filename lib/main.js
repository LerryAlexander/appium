...
import { default as getParser } from './parser';
import { getDefaultServerArgs } from './server-parser';
import { showConfig, checkNodeOk, validateServerArgs,
         warnNodeDeprecations, validateTmpDir, getNonDefaultArgs,
         getGitRev, APPIUM_VER } from './config';
import { validateDriversYaml, printAvailableDrivers } from './drivers';
import { runDriverCommand } from './driver-cli';
...
async function preflightChecks (parser, args, throwInsteadOfExit = false) {
  try {
    ...
    await validateDriversYaml(args.appiumHome);
    if (args.tmpDir) {
